...
module.exports = async (req, res) => {
    ...
    try {
        ...
        // Call 3: Fetch invoice detail to get invoice_items (not line_items!)
        ...
        if (targetInvoice) {
            ...
            console.log('Invoice detail invoice_items:', JSON.stringify(invoiceDetail.invoice_items, null, 2));
        }
        ...
        return res.status(200).json({
            ...
            invoice_detail_invoice_items: invoiceDetail?.invoice_items || null,
            invoice_detail_fields: invoiceDetail ? Object.keys(invoiceDetail) : [],
            device_items_found: invoiceDetail?.invoice_items?.filter(item =>
                item.name?.toLowerCase().includes('shield') ||
                item.name?.toLowerCase().includes('t10') ||
                item.name?.toLowerCase().includes('hotspot')
            ) || [],
            success: true
        });
    } catch (error) {
