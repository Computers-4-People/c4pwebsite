...
export default function Homepage() {
    ...
    return (
        <div className='font-sans'>
            <div style={{backgroundImage: `linear-gradient(to right, black 0%, rgba(0, 0, 0, 0.8) 30%, rgba(0, 0, 0, 0.5) 60%, transparent 100%),url('/homepage/background.jpeg')`}} className=" bg-cover h-screen bg-center bg-cover bg-relative bg-local bg-no-repeat ">
                <div class="ml-20 grid grid-cols-6 grid-rows-6 justify-items-stretch">
                    <div className='col-start-1 col-end-4 row-start-2 text-justify font-title text-8xl'>
                        <p class='text-c4p col-span-2 object-fill mb-5'>TRANSFORM A LIFE</p>
                    ...
                    <div className='mt-10 row-start-3 col-span-4 row-span-2 flex flex-row justify-items-start flex-wrap text-center text-xl w-128'>
                        <Link to="/ewaste" className="bg-c4p h-10 mr-3 h-11 rounded-md pt-2 px-5 hover:bg-c4p-hover hover:text-white">🤍 Donate your ewaste</Link>
                        <Link to="/programs" className='border-2 rounded-md border-white h-11 text-white h-10 pt-1.5 px-5 hover:bg-white hover:text-black'> Discover our programs </Link>
                    </div>
                </div>
            </div>
            <div>
                <Textmarquee text="10,000,000 hours of connectivity provided    3,000 computers donated" duration="10s" className="text-6xl" />
            </div>
            ...
            <div className='absolute inset-0 col-start-4 col-end-6 row-start-1 row-end-4 bg-contain bg-top bg-no-repeat z-0' style={{ backgroundImage: "url('../quotes.png')" }} />
